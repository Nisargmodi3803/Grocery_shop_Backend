...
{
    @Query("SELECT wishlist FROM Wishlist wishlist WHERE wishlist.customer.customerId = :customerId")
    List<Wishlist> findByCustomerId(int customerId);

    @Query("SELECT wishlist FROM Wishlist wishlist WHERE wishlist.customer.customerId = :customerId AND wishlist.product.id= :productId")
