...
{
    @Query("SELECT inquiry FROM ProductInquiry inquiry WHERE inquiry.isDeleted=1 AND (inquiry.customer.customerEmail = :customerEmail AND inquiry.customer.isDeleted=1 AND inquiry.product.is_deleted=1)")
